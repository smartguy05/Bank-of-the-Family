...
export const queryKeys = {
  ...
  peers: () => ["family", "peers"] as const,
  peerRequests: (status?: string) => ["peerRequests", status ?? "all"] as const,
  ious: (status?: string) => ["ious", status ?? "all"] as const,
  iou: (id: string) => ["ious", id] as const,
